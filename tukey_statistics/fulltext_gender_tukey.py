...
from visualization import gender_visualization
import pandas as pd

from util import path

def perform_tukey():
    collection_dict = gender_visualization.assemble_gender_collection()
    ...
    for perspective, dataframe in dataframe_dict.items():
        ...
        table_data = ''
        full_caption = 'Fulltext ' + perspective.lower() + ' tukey test'
        ...
        with open(path.data_root() / 'statistics' / 'fulltext' / perspective_path, 'w') as file:
            file.write(template)
